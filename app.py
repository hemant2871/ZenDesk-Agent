...
st.set_page_config(
    page_title="ZenDesk Support Agent",
...
with st.sidebar:
    st.markdown(
        ...
        '<p style="color:#a78bfa; font-weight:700; font-size:1.1rem; margin:0.3rem 0 0;">Zendesk Support Agent</p>'
        '<p style="color:rgba(255,255,255,0.4); font-size:0.75rem; margin:0;">v1.0</p>'
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.agent_ready:
        ...
        st.error(f"Init failed: {st.session_state.init_error[:120]}")
...
with col_form:
    ...
    if not st.session_state.agent_ready:
        st.info("Initialise the agent in the sidebar first.")
